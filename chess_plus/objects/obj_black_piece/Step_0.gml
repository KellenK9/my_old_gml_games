...
if(taken > 0){
	x = global.black_taken_x + ((taken mod 8) * 8)
	y = global.black_taken_y + (1.5 * (taken - (taken mod 8)))
	...
	if(!global.game_over){
		...
		if(curr_selected){
			...
			if(mouse_check_button(mb_any)){
				...
				instance_position_list(x, y, obj_black_piece, list, false)
				...
				if(move_valid){
					...
					if(position_meeting(x, y, obj_white_piece)){
						global.white_pieces_taken += 1
						taken_piece = instance_nearest(x, y, obj_white_piece)
						taken_piece.taken = global.white_pieces_taken
					}
				}
				else{
					x = original_x
